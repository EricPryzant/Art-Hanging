...
const ArtHangingCalculator = () => {
  ...
  const [artworks, setArtworks] = useState([
    { id: 1, width: 0, height: 0, wireOffset: 0 }
  ]);
  ...
  const updateArtwork = (id, field, value) => {
    setArtworks(artworks.map(art =>
      art.id === id ? { ...art, [field]: value === '' ? 0 : (parseFloat(value) || 0) } : art
    ));
  };
  ...
  const calculateNailPosition = () => {
    if (configuration === 'single') {
      ...
      const nailHeight = targetCentroid + artCentroid - art.wireOffset;
      ...
      return [{
        ...
        equation: `Nail Height = ${targetCentroid.toFixed(2)} + (${art.height.toFixed(2)} / 2) - ${art.wireOffset.toFixed(2)} = ${nailHeight.toFixed(2)}${unitLabel}`,
    ...
    if (configuration === 'custom') {
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          ...
          const nailHeight = actualCentroid + (art.height || 0) / 2 - (art.wireOffset || 0);
    ...
    if (configuration === 'vertical') {
      ...
      return artworks.map((art, i) => {
        ...
        const nailHeight = artCentroid + (art.height || 0) / 2 - (art.wireOffset || 0);
      ...
      return artworks.map((art, i) => {
        ...
        const nailHeight = targetCentroid + artCentroid - (art.wireOffset || 0);
        ...
        return {
          ...
          horizontalDistance: artCenterX.toFixed(2),
          horizontalFromEdge: 'left',
          equation: `Nail Height = ${targetCentroid.toFixed(2)} + (${(art.height || 0).toFixed(2)} / 2) - ${(art.wireOffset || 0).toFixed(2)} = ${nailHeight.toFixed(2)}${unitLabel}`,
